...
# ─────────────────────────── 本站配置 ───────────────────────────
SITE = "https://mojiokoshi.glspace.co.jp"
APP_NAME = "Mojiokoshi"
...
# 繁体的内部码是历史遗留的 zhHant（不是 zh-Hant），字典键沿用它，别改
LANGS = {
    ...
    "zhHant": dict(out="zh-hant", path="/zh-hant/", htmllang="zh-Hant", locale="zh_TW", font="Noto+Sans+TC", name="繁體中文"),
}
HREFLANG = {"ja": "ja", "en": "en", "zh": "zh-Hans", "zhHant": "zh-Hant"}

BASE_FONTS = []
ESCAPE_VALUES = False
BODY_CLASS = None
# 这个站的语言必须无条件跟随系统、绝不记住手动选择（用户明确要求过）。
# 所以不写 localStorage，根页面也不自动重定向，只给非日语访客一条可点提示。
STORAGE_KEY = None
BAR_KEY = {"en": "en", "zh": "zh", "zhHant": "zh-Hant"}
# 浏览器语言 → 本站内部语言码
DETECT = {"en": "en", "zh-Hans": "zh", "zh-Hant": "zhHant"}
DEFAULT_LANG = "ja"
EXTRA_URLS = ["/privacy.html", "/support.html"]
# ────────────────────────── 配置到此为止 ──────────────────────────
